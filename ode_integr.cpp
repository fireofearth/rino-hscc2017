...
void JTaylor(vector<vector<AAF>> &J_res, OdeVar odeVAR_x, OdeVar odeVAR_g, vector<vector<AAF>> &J0,
             vector<vector<AAF>> &J_rough, double tau, int order) {
    double taui = tau;
    vector<vector<AAF>> aux(sysdim, vector<AAF>(jacdim));
    vector<vector<AAF>> Jaci(sysdim, vector<AAF>(sysdim));
    //  iMatrix aux, Jaci;
 ...
 */
void init_nextstep_ode(vector<AAF> &x0, vector<AAF> &x0p1, vector<AAF> &x, vector<AAF> &xp1, vector<vector<AAF>> &J0,
                       vector<vector<AAF>> &Jtau) {
    for (int i = 0; i < sysdim; i++)
...
void
print_solutionstep_ode(vector<interval> &Xouter, vector<interval> &Xinner, vector<interval> &Xcenter, double tnp1) {
...
void HybridStep_ode::print_solutionstep(vector<interval> &Xouter, vector<interval> &Xouter_robust,
                                        vector<interval> &Xouter_minimal, vector<interval> &Xinner,
                                        vector<interval> &Xinner_robust, vector<interval> &Xinner_minimal,
                                        vector<interval> &Xcenter) {
    double aux, minwidth_ratio = 1.0;  // min on xi ( width of inner-approx (xi) / width of outer-approx (xi) )
    double mean_dist; // mean value on the xi of max distance between inner and outer approximations
    double tnp1 = tn + tau;
...
void HybridStep_ode::TM_evalandprint_solutionstep(vector<interval> &eps, double tnp1) {
    ...
    vector<interval> Xouter(sysdim), Xouter_robust(sysdim), Xouter_minimal(sysdim), Xinner(sysdim), Xinner_robust(
            sysdim), Xinner_minimal(sysdim), Xcenter(sysdim);
    ...
    InnerOuter(Xinner, Xinner_robust, Xinner_minimal, Xouter, Xouter_robust, Xouter_minimal, TMcenter.xp1, TMJac.Jp1,
               eps);
